...
In this blogpost, we will start building a simple ETL pipeline using the https://neo-technology-field.github.io/python-etl-lib/index.html[Neo4j-ETL-Lib]. The pipeline will load some fictionary CSV data. Concepts and ideas will be explained along the way.
...
== Data Model
Our target data model we are going to use is pictured below:

[#img-schema, role="img-responsive"]
.data model
image::img/e-commerce.png[data model]

Again, all very simple. Focus is on how to use the etl-lib and the benefits it hopefully brings.
...
While this is fine and can be extended with batching, it is hard to run autonomously, trace executions, failures and so on.

To load the same data with the library, we need to wrap that inside a `Task` derived class.

The etl-lib provides multiple such classes to load data from various data sources. One such class is `CSVLoad2Neo4jTask`. Below code uses that class to load exactly the same data:
...
<2> The etl-lib work with batches for improved performance, hence we need to `UNWIND` the batches in the first line. The batch size defaults to 5000 but can be configured via init param.


== Building the pipeline
The above `LoadProducts` class does run on its own, it needs to be part of a pipeline and have access to a `ETLContext` instances, which provides access to databases, logging and so on:
...
<3> Create a context from the parameters provided commandline or environment parameters. The context is used to pass access to infrastructure objects such as databases and logging.
...
* Reporting. Right now, only to the logfile configured. We will enable reporting to a database in the next example.
...
As you can see, the `Task`s form a tree. Each `Task` and `TaskGroup` reports some statistics. With the last one the overall statistics for the pipeline.
...
== Reporting to a a database
By simply setting `REPORTER_DATABASE=neo4j` in the environment (`.env` file), the reporter will write these statistics to a database. The database pointed could be a separate from the database we load the data into.
The log will report with database it is using:
...
This graph is updated with each batch processed. So, having long-running pipelines, you can see the progress. The etl-lib also provides a Neo4j dashboard that provides overview and details on etl runs. The imported `@cli` also added cli commands to query and manage statistics:
[code,bash]
----
python pipeline.py
